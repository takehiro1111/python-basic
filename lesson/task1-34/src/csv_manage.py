...
class ImportCSVManager(CSVManager):
    ...
    def _get_import_csv_file_path(self) -> str:
        has_datetime_now = self._create_time_stamp()
        import_csv_file_path = os.path.join(self.import_dir, f"import_items_{has_datetime_now}.csv")
    ...
    def _get_complete_csv_file_path(self) -> str:
        has_datetime_now = self._create_time_stamp()
        complete_csv_file_path = os.path.join(self.complete_dir, f"import_items_{has_datetime_now}.csv")

        return complete_csv_file_path

    def _read_import_csv_file(self):
        import_file_list = [file for file in os.listdir(self.import_dir) if os.path.isfile(os.path.join(self.import_dir, file))]

        rows = []
        for file in import_file_list:
            with open(os.path.join(self.import_dir, file),encoding = 'utf-8', newline = "") as csv_file:
